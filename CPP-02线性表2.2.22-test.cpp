...
{
    int len1 = GetLength(A), len2 = GetLength(B), DisAB = abs(len1 - len2);
    LinkList longlist, shortlist;
    if (len1 > len2)
    {
        longlist = A->next;
...
int main()
{
    vector<char> dataA;
    vector<char> dataB;
    string Word0{"fulltime"}, Word1{"time"};
    for (size_t i = 0; i < Word0.length(); i++)
    {
        dataA.push_back(Word0[i]);
    }
    for (size_t i = 0; i < Word1.length(); i++)
    {
        dataB.push_back(Word1[i]);
    }
    LinkList(A);
    InitList(A);
    A = CreateList(dataA);
